...
function mock(_ref) {
  ...
  return (0, _nock2.default)(/\.authy\.com/).filteringPath(path => path.replace(/key=.*?(&|$)/, 'key={key}$1').replace(/[\w]{8}(-[\w]{4}){3}-[\w]{12}/, '{id}')).get('/onetouch/json/approval_requests/{id}').query(request.query ? (0, _lodash.defaults)({ api_key: '{key}' }, request.query) : true).reply(response.code, response.body);
}
