...
function mock(_ref) {
  ...
  return (0, _nock2.default)(/\.authy\.com/).filteringPath(path => path.replace(/key=.*?(&|$)/, 'key={key}$1').replace(/\/[0-9].*\?/, '/{authyId}?')).get('/protected/json/call/{authyId}').query(request.query ? (0, _lodash.defaults)({ api_key: '{key}' }, request.query) : true).reply(response.code, response.body);
}
