...
function mock(_ref) {
  ...
  return (0, _nock2.default)(/\.authy\.com/).filteringPath(path => path.replace(/\d+/, '{authyId}')).filteringRequestBody(body => body.replace(/key=.*?(&|$)/, 'key={key}$1')).post('/onetouch/json/users/{authyId}/approval_requests', request.body).reply(response.code, response.body);
}
/**
